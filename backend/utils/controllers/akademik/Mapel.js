...
exports.readDataId = (req, res) => {
  // query sql
    const querySql = 'SELECT a.id_kurikulum, nm_kurikulum, kd_mapel, nm_mapel, a.kd_jurusan, nm_jurusan, a.nip, nm_guru, tingkat, a.kptsi_umum, a.kptsi_khusus, jml_jam, a.id_kmapel, nm_kmapel, a.status FROM mapel a LEFT JOIN k_mapel b ON a.id_kmapel=b.id_kmapel LEFT JOIN guru c ON a.nip=c.nip LEFT JOIN jurusan d ON a.kd_jurusan=d.kd_jurusan LEFT JOIN kurikulum e ON a.id_kurikulum=e.id_kurikulum WHERE kd_mapel = ?';
